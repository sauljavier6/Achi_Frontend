...
export default function AuthLayout() {
  return <main className="grid min-h-screen bg-background lg:grid-cols-[1.05fr_.95fr]">
    <section className="relative hidden overflow-hidden bg-primary p-12 text-white lg:flex lg:flex-col lg:justify-between">
      ...
      <div className="relative flex items-center gap-3"><img src="/logo.png" alt="Hachi Veterinaria" className="h-14 w-14 rounded-2xl bg-white object-cover" /><div><p className="text-2xl font-extrabold">Hachi Veterinaria</p><p className="text-sm text-white/70">Administración y punto de venta</p></div></div>
    ...
    <section className="flex min-h-screen items-center justify-center px-4 py-8 sm:px-8">
      <div className="w-full max-w-md"><div className="mb-8 flex items-center justify-center gap-3 lg:hidden"><img src="/logo.png" alt="Hachi Veterinaria" className="h-12 w-12 rounded-2xl object-cover" /><div><p className="font-extrabold text-primary">Hachi Veterinaria</p><p className="text-xs text-on-surface-variant">Punto de venta</p></div></div><Outlet /></div>
    </section>
  </main>;
}
