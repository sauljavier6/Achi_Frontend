...
export const getProductsCatalogoBanner = async ({ page = 1, limit = 10 }) => {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/ecommerce/products?page=${page}&limit=${limit}`,
    {
      method: "GET",
      headers: {
...
export const getProductById = async (id: number) => {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/ecommerce/products/${id}`,
    {
      headers: {
...
export const getProductsByDescription = async (description: string) => {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/ecommerce/products/searchproduct/${description}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
