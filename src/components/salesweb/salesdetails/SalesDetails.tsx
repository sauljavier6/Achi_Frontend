...
interface SaleProduct { ID_SaleProduct: number; Quantity: number; TaxBase?: number | string; TaxAmount?: number | string; Product: { Description: string }; Stock: { Description: string; Saleprice: string } }
type OrderStatus = "RECEIVED" | "PROCESSING" | "READY" | "OUT_FOR_DELIVERY" | "READY_FOR_PICKUP" | "DELIVERED" | "CANCELLED";
interface Venta { ID_Sale: number; Total: string; Subtotal: string; Iva: string; Envio?: string; StateWeb: boolean; OrderStatus?: OrderStatus; createdAt: string; Cliente?: { Name: string; Email?: { Description?: string }; Phone?: { Description?: string } }; Address?: { Description?: string }; SaleProduct?: SaleProduct[] }
...
const SalesDetailComponent = ({ sale, onCompleted }: Props) => {
  ...
  const handleStatus = async (status: OrderStatus) => {
    if (!sale || isCompleting || status === data?.OrderStatus) return;
    if ((status === "DELIVERED" || status === "CANCELLED") && !window.confirm(status === "DELIVERED" ? "¿Confirmas que el pedido fue entregado?" : "¿Confirmas la cancelación del pedido?")) return;
    setIsCompleting(true);
    try {
      const response = await pathStateWebSale(sale, status);
      if (!response.success) throw new Error(response.message || "No fue posible actualizar el pedido");
      await Promise.all([queryClient.invalidateQueries({ queryKey: ["sales"] }), queryClient.invalidateQueries({ queryKey: ["salewebbyid", sale] })]);
      toast.success(response.message || "Estado del pedido actualizado");
      if (status === "DELIVERED" || status === "CANCELLED") onCompleted?.();
    } catch (err) { toast.error(err instanceof Error ? err.message : "No fue posible actualizar el pedido"); }
  ...
  const snapshotBase = items.reduce((sum, item) => sum + Number(item.TaxBase || 0), 0);
  const snapshotTax = items.reduce((sum, item) => sum + Number(item.TaxAmount || 0), 0);
  const displayedTax = snapshotTax > 0 ? snapshotTax : Number(data.Iva || 0);
  const displayedSubtotal = snapshotBase > 0 ? snapshotBase : Math.max(0, Number(data.Total) - Number(data.Envio || 0) - displayedTax);
  const currentStatus: OrderStatus = data.OrderStatus || (data.StateWeb ? "RECEIVED" : "DELIVERED");
  const statusLabels: Record<OrderStatus, string> = { RECEIVED: "Pedido recibido", PROCESSING: "En preparación", READY: "Surtido y listo", OUT_FOR_DELIVERY: "En camino", READY_FOR_PICKUP: "Listo para recolección", DELIVERED: "Entregado", CANCELLED: "Cancelado" };

  return <div className="space-y-5">
    <header className="flex flex-col gap-3 border-b border-slate-200 pb-5 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold text-[#c70063]">Recepción de pedidos</p><h2 className="mt-1 text-2xl font-bold text-slate-900 sm:text-3xl">Pedido {formatFolio(data.ID_Sale)}</h2><p className="mt-1 text-sm text-slate-500">Recibido el {dateTime(data.createdAt)}</p></div><span className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-sm font-bold ${currentStatus === "CANCELLED" ? "bg-red-50 text-red-700" : currentStatus === "DELIVERED" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{currentStatus === "DELIVERED" ? <CheckCircle2 size={17} /> : <ClipboardCheck size={17} />}{statusLabels[currentStatus]}</span></header>
    <section aria-label="Progreso del pedido" className="grid grid-cols-3 overflow-hidden rounded-xl border border-slate-200 bg-slate-50"><div className="flex flex-col items-center gap-1 border-r border-slate-200 px-2 py-3 text-center text-[#007782]"><CheckCircle2 size={20} /><span className="text-xs font-bold sm:text-sm">Pago confirmado</span></div><div className={`flex flex-col items-center gap-1 border-r border-slate-200 px-2 py-3 text-center ${currentStatus !== "DELIVERED" && currentStatus !== "CANCELLED" ? "bg-white text-[#c70063]" : "text-[#007782]"}`}><PackageCheck size={20} /><span className="text-xs font-bold sm:text-sm">Preparar pedido</span></div><div className={`flex flex-col items-center gap-1 px-2 py-3 text-center ${currentStatus === "DELIVERED" ? "bg-white text-[#007782]" : "text-slate-400"}`}><Truck size={20} /><span className="text-xs font-bold sm:text-sm">Entregado</span></div></section>
    ...
    <section className="ml-auto rounded-xl bg-slate-50 p-4 sm:max-w-md sm:p-5"><div className="space-y-2 text-sm"><div className="flex justify-between text-slate-600"><span>Subtotal</span><span>{money(displayedSubtotal)}</span></div><div className="flex justify-between text-slate-600"><span>IVA incluido</span><span>{money(displayedTax)}</span></div><div className="flex justify-between text-slate-600"><span>Envío</span><span>{money(data.Envio)}</span></div><div className="mt-3 flex justify-between border-t border-slate-200 pt-3 text-xl font-bold"><span>Total</span><span>{money(data.Total)}</span></div></div></section>
    <section className="rounded-xl border border-[#007782]/20 bg-[#007782]/5 p-4"><label className="block text-sm font-bold text-slate-800">Actualizar estado y notificar al cliente</label><p className="mt-1 text-xs text-slate-600">Al cambiar el estado, enviaremos un correo con el avance del pedido.</p><div className="mt-3 flex flex-col gap-3 sm:flex-row"><select value={currentStatus} onChange={(event) => handleStatus(event.target.value as OrderStatus)} disabled={isCompleting} className="min-h-12 flex-1 rounded-xl border border-slate-300 bg-white px-4 disabled:opacity-60"><option value="RECEIVED">Pedido recibido</option><option value="PROCESSING">En preparación</option><option value="READY">Surtido y listo</option><option value="OUT_FOR_DELIVERY">En camino a domicilio</option><option value="READY_FOR_PICKUP">Listo para recolección</option><option value="DELIVERED">Entregado</option><option value="CANCELLED">Cancelado</option></select></div></section>
    <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end"><button onClick={handlePrintRemision} disabled={isPrinting} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-[#007782] bg-white px-5 font-bold text-[#007782] hover:bg-[#007782]/5 disabled:opacity-50"><Download size={18} />{isPrinting ? "Generando…" : "Descargar remisión"}</button></div>
  </div>;
};
