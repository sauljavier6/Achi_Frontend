...
export default function CajasPays({ ID_Sale }: { ID_Sale: number }) {
  ...
  return (
    <div className="w-full min-w-0 space-y-5">
      ...
      {draftPayments.length > 0 && <section className="rounded-2xl border border-[#c70063]/20 bg-[#c70063]/[0.03] p-4 sm:p-5"><h3 className="font-bold text-slate-900">Pagos por registrar</h3><div className="mt-3 space-y-2">{draftPayments.map((payment, index) => <div key={`${payment.ID_Payment}-${index}`} className="grid min-w-0 grid-cols-1 gap-2 rounded-xl bg-white p-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center sm:gap-4"><div className="min-w-0"><p className="truncate font-semibold">{payment.Description}</p><p className="truncate text-xs text-slate-500">{payment.ReferenceNumber || "Sin referencia"}</p></div><strong className="sm:text-right">{money(payment.Monto)}</strong><button type="button" onClick={() => setDraftPayments((items) => items.filter((_, itemIndex) => itemIndex !== index))} className="w-fit text-sm font-semibold text-red-600">Quitar</button></div>)}</div><div className="mt-4 flex flex-wrap justify-between gap-2 border-t border-[#c70063]/10 pt-3"><span>Saldo después del abono</span><strong>{money(remaining)}</strong></div></section>}
